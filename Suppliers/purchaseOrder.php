...
<?php 
    ...
    echo "<br>".$sid=$_GET['id'];
        ...
        $qry="SELECT * FROM purchase_order WHERE dateOrdered='$dater' AND supplierID='$sid'";
            $drun=mysqli_query($con,$qry);
            $drow=mysqli_fetch_array($drun);
            $ponum=$drow['PONum'];

            $purchase="SELECT * FROM ordered_items WHERE PONum='$ponum' AND quantity !='0'";
            $rrun=mysqli_query($con,$purchase);
...
<section id="main-content" style="padding-left: 4%; padding-right: 2%;">
    ...
    <section class="wrapper">
         <a href=" viewSuppliersProfile.php?id=<?php echo $_SESSION['supplier'];?>" style="">  <!--2 >> navigate to memberList <3 -->
		    ...
		    <div class="row">
				    <div class="col-lg-12" >
					      <h1 class="page-header"><br>Puchase Ordered on <?php echo formatdate($dater); ?> </h1>
                           <?php 
        ...
        <div class="row">
            <div class="col-lg-12">
                <section class="panel">
                    <div>
                        <table class="table" id="dataTable" style="font-size: 24px">
                            ...
                            while($rrow= mysqli_fetch_assoc($rrun)){
                                 ...
                                 $item = "SELECT * FROM inventory WHERE productNum='$pno'";
                                ...
                                while($row =mysqli_fetch_array($irun)){ ?> 
                                    ...
                                    <?php $totalPurchase=$totalPurchase+$row['originalPrice']; ?>
                                </tr>
                                <?php }
                            } ?>
                        </tbody>
                 ...
                 <div style="text-align:center; font: 36px 'Aleo';">Total : <?php echo "<b>".formatMoney($totalPurchase)."</b>"; ?></div>
            </section>
        </div>
    </div>
     <div class="btn-group">
